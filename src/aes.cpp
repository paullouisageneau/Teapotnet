...
void Aes::setEncryptionKey(const char *key, size_t size)
{
	setEncryptionKey(key, size, mEncryptionKey);
}

void Aes::setEncryptionKey(const char *key, size_t size, Aes::Key &out)
{
	Assert(key);
...
void Aes::setDecryptionKey(const char *key, size_t size)
{
	setDecryptionKey(key, size, mDecryptionKey);
}

void Aes::setDecryptionKey(const char *key, size_t size, Aes::Key &out)
{
...
void Aes::setEncryptionInit(const char *iv)
{
	std::memcpy(mEncryptionInit, iv, AES_BLOCK_SIZE); 
}

void Aes::setDecryptionInit(const char *iv)
{
 ...
 */
void Aes::encrypt(char *in, char *out)
{
	Assert(in && out);
 ...
 * in and out can overlap
 */
void Aes::decrypt(char *in, char *out)
{
	Assert(in && out);
...
size_t Aes::readData(char *buffer, size_t size)
{
	Assert(buffer);
	...
	while(size)
	{
		...
		std::memcpy(buffer, mTempBlockOut, count);
		
		mTempBlockOutSize-= count;
		buffer+= count;
		size-= count;
		total+= count;
		
		if(mTempBlockOutSize)
			std::memmove(mTempBlockOut, mTempBlockOut+count, mTempBlockOutSize);
	}
	
	return total;
}

size_t Aes::readBlock(char *out)
{
...
void Aes::writeData(const char *data, size_t size)
{
	Assert(data);
