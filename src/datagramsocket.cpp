...
void DatagramSocket::bind(const Address &local, bool broadcast)
{
	...
	try {
		...
		if(broadcast) setsockopt(mSock, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<void*>(&enabled), sizeof(enabled));
		if(local.addrFamily() == AF_INET6) setsockopt(mSock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<void*>(&disabled), sizeof(disabled)); 
