...
Index *Index::GlobalInstance = NULL;
const String Index::CacheDirectoryName = "_cache";
const String Index::UploadDirectoryName = "_upload";
	...
	mRunning(false)
{
	...
	if(File::Exist(mFileName))
	{
		...
		{
			LogWarn("Index", String("Unable to load directories: ") + e.what());
		}
	}
...
User *Index::user(void) const
{
	return mUser; 
}

String Index::userName(void) const
{
	if(mUser) return mUser->name();
	else return "";
}

void Index::addDirectory(const String &name, String path, Resource::AccessLevel level)
{
	Synchronize(this);
...
void Index::removeDirectory(const String &name)
{
  	Synchronize(this);
...
void Index::getDirectories(Array<String> &array) const
{
	Synchronize(this);
...
void Index::setDirectoryAccessLevel(const String &name, Resource::AccessLevel level)
{
	mDirectoriesAccessLevel[name] = level;
}

Resource::AccessLevel Index::directoryAccessLevel(const String &name) const
{
...
bool Index::moveFileToCache(String &fileName, String name)
{
	Synchronize(this);
	...
	{
		LogDebug("Index", "File is too large for cache: " + name);
		return false;
	}
	...
	{
		// This is not normal
		LogWarn("Index", "Not enough free space in cache for " + name);
		return false;
	}
	
	if(name.empty()) name = fileName.afterLast(Directory::Separator);
	LogInfo("Index", "Moving to cache: " + name);
...
void Index::start(void)
{
	Synchronize(this);
	if(!mRunning) Scheduler::Global->schedule(this);
}

bool Index::query(const Resource::Query &query, Resource &resource)
{
	Synchronize(this);
...
bool Index::query(const Resource::Query &query, Set<Resource> &resources)
{
	Synchronize(this);
...
void Index::http(const String &prefix, Http::Request &request)
{
				...
				if(this != GlobalInstance && request.method == "POST")
				{
			  		...
			  		if(command == "delete")
					{
						...
						++it)
						{
							...
							LogInfo("Index::Http", String("Uploaded: ") + fileName);
							...
							{
								LogWarn("Index::Http", String("Unable to get resource after upload: ") + e.what());
							}
						}
	...
	{
		LogWarn("Index::http", e.what());
		throw 500;	// Httpd handles integer exceptions
	}
}

bool Index::getResource(const BinaryString &digest, Resource &resource)
{
	Synchronize(this);
...
void Index::insertResource(const BinaryString &digest, const String &path)
{
	Synchronize(this);
	...
	if(selectStatement.step())
	{
		...
		LogInfo("Index", String("Registering: ") + path);
...
bool Index::prepareQuery(Database::Statement &statement, const Resource::Query &query, const String &fields, bool oneRowOnly)
{
...
void Index::update(const String &url, String path, int64_t parentId, bool computeDigests)
{
	Synchronize(this);
		...
		if(statement.step())	// entry already exists
		{
			...
			if(time == dbTime && size == dbSize && type == dbType && !(type && digest.empty()))
			{
				...
				statement.execute();
			  	...
			  	if(computeDigests) LogInfo("Index", String("Processing: ") + path);
			...
			statement.finalize();
			if(computeDigests) LogInfo("Index", String("Processing new: ") + path);
			else LogInfo("Index", String("Indexing: ") + path);
	...
	{
		LogWarn("Index", String("Processing failed for ") + path + ": " + e.what());

	}
}

String Index::urlToDirectory(const String &url) const
{
	if(url.empty() || url[0] != '/') throw Exception("Invalid URL");
	return String(url.substr(1)).before('/');
}

String Index::urlToPath(const String &url) const
{
	if(url.empty() || url[0] != '/') throw Exception("Invalid URL");
	Synchronize(this);
...
String Index::absolutePath(const String &path) const
{
...
bool Index::isHiddenUrl(const String &url) const
{
...
Resource::AccessLevel Index::urlAccessLevel(const String &url) const
{
	return directoryAccessLevel(urlToDirectory(url));	
}

int64_t Index::freeSpace(String path, int64_t maxSize, int64_t space)
{
	int64_t totalSize = 0;
...
void Index::run(void)
{
	...
	try {
		...
		{
			LogWarn("Index", String("Update failed for directory ") + names[i] + ": " + e.what());
		}
		...
		{
			LogWarn("Index", String("Hashing failed for directory ") + names[i] + ": " + e.what());

		}
		
		LogDebug("Index::run", "Finished");
	}
	catch(const Exception &e)
	{
		LogWarn("Index::run", e.what());
	}
...
/*
void Index::keywords(String name, Set<String> &result)
{
